...
```bash
npm run test:race       # 50 параллельных вебхуков, разные event_id, вебхук раньше заказа, повтор после доставки
npm run test:recovery   # пустой пул -> восстановимое состояние -> ручная повторная выдача, идемпотентность
npm run test:promo      # лимит промокода под 20 параллельными запросами
```
...
- ✅ Этап 1 — витрина по макету (шапка, баннер-карусель, иконки сервисов, блок пополнения Steam, ряд карточек) + полный флоу создание заказа → оплата (вебхук-заглушка) → автоматическая выдача → статус заказа
- ✅ Этап 2 — однократная выдача под гонками, покрыто `test:race`
- ✅ Этап 3 — сбои и восстановление, админ-ручки, покрыто `test:recovery`
- ✅ Этап 4 — промокоды с лимитом под гонкой, покрыто `test:promo`
- ✅ 5 точек интерактива по макету: карусель, меню «Каталог», переключатель валют, hover на иконках сервисов, hover на карточках товара

## Сколько времени ушло по факту
